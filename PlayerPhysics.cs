...
public class PlayerPhysics : MonoBehaviour {
	...
	private int collisionDivisionsX = 5;//3;
	private int collisionDivisionsY = 8;
	...
	public void Move(Transform t, Vector2 moveAmount) {
			...
			if (Physics.Raycast(ray,out hit,Mathf.Abs(deltaY) + skin,collisionMask)) {
				...
				if (dst > skin) {
					...
					if(hit.collider.gameObject.tag == "Platform"){
						...
						//deltaY = dst * dir - skin * dir + 0.2f;
					}
				}
				else {


					//Delta non zero to stop it falling through surface
					if(isOrb){
						if(dir < 0){
							deltaY = 0.005f;
						} else if (dir > 0) {
	...
	// Set collider
	public void SetColliderCentre( Vector3 centre) {
		collider.center = centre;
		c = centre * colliderScale;
	}
